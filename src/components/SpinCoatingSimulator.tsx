...
function gelReasonLabel(
  reason: "concentration" | "meyerhofer" | "max_time",
): string {
  switch (reason) {
    case "concentration":
      return "Solid concentration C ≥ 1";
    case "meyerhofer":
      return "Meyerhofer limit h ≤ h_f";
    default:
...
export default function SpinCoatingSimulator() {
  ...
  const waferProfile = useMemo(
    () => buildWaferCrossSection(coreResult.hFinal, radiusM),
    [coreResult.hFinal, radiusM],
  );
          ...
          <div className="sticky top-0 max-h-[50vh] overflow-y-auto p-5 lg:max-h-screen">
            ...
            <div className="mt-6 rounded-lg border border-slate-800 bg-slate-900/50 p-3 text-xs text-slate-500">
              ...
              <ul className="space-y-1 font-mono text-slate-400">
                ...
                <li>C₀ = {FIXED_C0}</li>
                <li>Edge bead peak = {(EDGE_BEAD_PEAK_FRACTION * 100).toFixed(0)}% × h_final</li>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-xl border border-slate-800 bg-slate-900/40 p-4">
        ...
        <p className="mb-3 text-xs text-slate-500">
          Edge bead ≈ {(EDGE_BEAD_PEAK_FRACTION * 100).toFixed(0)}% of h_final (
          {beadAmplitudeNm.toFixed(1)} nm peak) from 90% R ({(0.9 * radiusMm).toFixed(1)} mm) ·
          ±2% spec on central field (r &lt; 95% R)
        </p>
