...
/** Edge-bead peak ≈ 4% of final center thickness (Fab typical 3–5%) */
export const EDGE_BEAD_PEAK_FRACTION = 0.04;
...
export function edgeBeadAmplitudeNm(hFinalM: number): number {
  return hFinalM * NM_PER_M * EDGE_BEAD_PEAK_FRACTION;
}
...
function crossedGelation(
  ...
  // Meyerhofer: film thins *to* h_f; skip if equilibrium thickness exceeds h₀
  if (
    useMeyerhofer &&
    hFAnalytic > 0 &&
    hFAnalytic < h0 &&
...
/** Gaussian edge bead from 90% radius; amplitude scales with h_final (Fab 3–5%) */
export function buildWaferCrossSection(
  ...
  const beadStart = 0.9 * radiusM;
  const sigma = beadSigmaFraction * radiusM;
  const beadAmplitudeNm = edgeBeadAmplitudeNm(hUniformM);
  ...
  for (let i = 0; i <= sampleCount; i++) {
    ...
    const inUniformityZone = r < beadStart;
    const deviation =
